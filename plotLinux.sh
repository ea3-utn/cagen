...
#
#--------------------------------------------------------------------------------------------------------------
# Script de control | Obtener en tiempo real la evolucion de los funcionales obtenidos
#--------------------------------------------------------------------------------------------------------------
#
# --------  script -------------------------------------------------------------------------------------------

cat mess.mess | grep fonctionnelle | sed 's/--> fonctionnelle ://g' | sort -t. -k 1,1n -k 2,2n -k 3,3n -k 4,4n -k 5,5n -k 6,6n -k 7,7n -k 8,8n -k 9,9n | awk '{print NR,$0}' $1 >/tmp/coordenadas.plot

gnuplot -persist -e "plot '/tmp/coordenadas.plot' using 1:2"
